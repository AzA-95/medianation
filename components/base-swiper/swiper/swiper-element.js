...
(function () {
	...
	function getParams(element, propName, propValue) {
		...
		const allowedParams = localParamsList.map((key) =>
			key.replace(/_/, ''),
		);
		...
		attrsList.forEach((attr) => {
			...
			if (moduleParam) {
				...
				passedParams[parentObjName][subObjName] = formatValue(
					attr.value,
				);
			} else {
	...
	class SwiperContainer extends ClassToExtend {
		...
		calcSlideSlots() {
			...
			const slideSlotChildren = [
				...
				return parseInt(
					child.getAttribute('slot').split('slide-')[1],
					10,
				);
			});
		...
		render() {
			...
			if (this.slideSlots > 0) {
				localStyles = localStyles.replace(
					/::slotted\(([a-z-0-9.]*)\)/g,
					'$1',
				);
			}
