...
!(function () {
	...
	function c() {
		...
		for (let i = 1; i < arguments.length; i += 1) {
			...
			if (
				...
				const s = Object.keys(Object(r)).filter(
					(e) => t.indexOf(e) < 0,
				);
	...
	var L = {
			...
			for (let i = 0; i < u; i += 1) {
				...
				if (
					...
					if ('auto' === s.slidesPerView) {
						...
						(k = (n - (s.slidesPerView - 1) * x) / s.slidesPerView),
							s.roundLengths && (k = Math.floor(k)),
							c[i] &&
		...
		updateActiveIndex: function (e) {
			...
			if (
				(l >= i.length && (l = i.length - 1), d === n && !t.params.loop)
			)
				return void (
		...
		slideTo: function (e, t, s, i, r) {
			...
			if (
				...
				return (
					...
					'reset' !== b &&
						(n.transitionStart(s, b), n.transitionEnd(s, b)),
					!1
				);
			if (o.cssMode) {
				const e = n.isHorizontal(),
	...
	function G(e) {
		...
		if ('touchstart' === r.type)
			return void A(t, r, r.targetTouches[0].pageX);
	...
	function D(e) {
		...
		if (p.preventedByNestedSwiper)
			return (a.startX = u), void (a.startY = h);
		if (!s.allowTouchMove)
			return (
		...
		if (
			s.params.threshold &&
			Math.sqrt(f ** 2 + m ** 2) < s.params.threshold
		)
			return;
		if (void 0 === r.isScrolling) {
			...
			(s.isVertical() && a.currentX === a.startX)
				...
				: f * f + m * m >= 25 &&
					((e =
						(180 * Math.atan2(Math.abs(m), Math.abs(f))) / Math.PI),
					...
					r.currentTranslate > s.minTranslate() &&
						...
						n.resistance &&
							(r.currentTranslate =
								s.minTranslate() -
								1 +
								(-s.minTranslate() + r.startTranslate + v) **
									y)))
				: v < 0 &&
					(b &&
						S &&
					...
					r.currentTranslate < s.maxTranslate() &&
						...
						n.resistance &&
							(r.currentTranslate =
								s.maxTranslate() +
								1 -
								(s.maxTranslate() - r.startTranslate - v) **
									y))),
		...
		if (h > n.longSwipesMs) {
			...
			(r.target === t.navigation.nextEl ||
				...
				? r.target === t.navigation.nextEl
					...
					: t.slideTo(v)
				: ('next' === t.swipeDirection &&
						t.slideTo(null !== w ? w : v + T),
					'prev' === t.swipeDirection &&
						t.slideTo(null !== b ? b : v));
		}
	}
	function V() {
		const e = this,
	...
	function se(e, t, s) {
		...
		return (
			'string' == typeof t &&
				void 0 !== s &&
				o.push({ name: t, value: Z(s) ? { ...s } : s }),
			o.forEach((e) => {
				const t = te.filter((t) => 0 === e.name.indexOf(`${t}-`))[0];
				if (t) {
					const s = Q(t),
						i = Q(e.name.split(`${t}-`)[1]);
					void 0 === r[s] && (r[s] = {}),
						!0 === r[s] && (r[s] = { enabled: !0 }),
						(r[s][i] = ee(e.value));
				} else {
					...
					const s = ee(e.value);
					r[t] && te.includes(e.name) && !Z(s)
						? (r[t].constructor !== Object && (r[t] = {}),
							(r[t].enabled = !!s))
						: (r[t] = s);
				}
			}),
	...
	class oe extends re {
		...
		initialize() {
			...
			(this.swiperParams = t),
				...
				this.render(),
				(this.swiper = new U(this.shadowRoot.querySelector('.swiper'), {
					...(t.virtual
						? {}
						: {
								observer: !0,
		...
		static get observedAttributes() {
			return K.filter((e) => e.includes('_')).map((e) =>
				e
					.replace(/[A-Z]/g, (e) => `-${e}`)
					.replace('_', '')
					.toLowerCase(),
	...
	'undefined' != typeof window &&
		...
		'undefined' != typeof window &&
			(window.customElements.get('swiper-container') ||
				window.customElements.define('swiper-container', oe),
