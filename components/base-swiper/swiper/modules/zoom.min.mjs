...
function Zoom(e) {
		...
		r >= 0 && (c[r] = e),
			c.length < 2 ||
				...
				d.imageEl &&
					...
					a.scale > d.maxRatio &&
						(a.scale =
							d.maxRatio - 1 + (a.scale - d.maxRatio + 1) ** 0.5),
					a.scale < i.minRatio &&
						(a.scale =
	...
	function Y(e) {
		...
		(u.currentX =
			...
			(p.x =
				(u.touchesCurrent.x - p.prevPositionX) /
				(Date.now() - p.prevTime) /
				2),
			(p.y =
				(u.touchesCurrent.y - p.prevPositionY) /
				(Date.now() - p.prevTime) /
				2),
		...
		(i.scale = W || b),
			...
			!e || (1 === l && W)
				...
				: ((M = d.slideEl.offsetWidth),
					...
					(f = d.imageEl.offsetHeight),
					(x = v * i.scale),
					(X = f * i.scale),
					(Y = Math.min(M / 2 - x / 2, 0)),
					...
					h < y && (h = y),
					h > C && (h = C)),
			W && 1 === i.scale && ((d.originX = 0), (d.originY = 0)),
	...
	function W() {
		...
		t.wrapperEl.addEventListener('pointerdown', f, i),
	...
	function b() {
		...
		e.enabled = !1;
		const { passiveListener: i, activeListenerWithCapture: a } = w();
		t.wrapperEl.removeEventListener('pointerdown', f, i),
