...
function EffectCreative(e) {
	...
	effectInit({
		...
		on: s,
		setTranslate: () => {
			const { slides: e, wrapperEl: r, slidesSizesGrid: s } = t,
			...
			for (let r = 0; r < e.length; r += 1) {
				const s = e[r],
					n = s.progress,
					c = Math.min(
						Math.max(s.progress, -i.limitProgress),
						i.limitProgress,
					);
				let f = c;
				l ||
					(f = Math.min(
						Math.max(s.originalProgress, -i.limitProgress),
						i.limitProgress,
					));
				...
				c < 0
					...
					p.forEach((e, t) => {
						p[t] =
							`calc(${e}px + (${a(g.translate[t])} * ${Math.abs(c * o)}))`;
					}),
					d.forEach((e, r) => {
				...
				if ((h && g.shadow) || !h) {
					...
					if (
						(!e && g.shadow && (e = createShadow('creative', s)), e)
					) {
