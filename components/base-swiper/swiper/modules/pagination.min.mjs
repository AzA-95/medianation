	...
	function d() {
		...
		const d =
				a.virtual && a.params.virtual.enabled
					...
					: a.slides.length,
			m = a.params.loop
				? Math.ceil(d / a.params.slidesPerGroup)
				: a.snapGrid.length;
		if (
			(a.params.loop
				...
				(s.dynamicBullets &&
					...
					s.dynamicMainBullets > 1 &&
						...
						r > s.dynamicMainBullets - 1
							...
							: r < 0 && (r = 0)),
					(o = Math.max(l - r, 0)),
					(d = o + (Math.min(t.length, s.dynamicMainBullets) - 1)),
					(m = (d + o) / 2)),
				t.forEach((e) => {
					const a = [
						...[
		...
		c.forEach((e, i) => {
			if (
				('fraction' === s.type &&
					...
					e
						.querySelectorAll(classesToSelector(s.totalClass))
						.forEach((e) => {
				...
				'horizontal' === t ? (n = i) : (r = i),
					e
						.querySelectorAll(
							classesToSelector(s.progressbarFillClass),
						)
						.forEach((e) => {
		...
		if ('bullets' === e.type) {
			...
			for (let s = 0; s < l; s += 1)
				e.renderBullet
					...
					: (i += `<${e.bulletElement} ${a.isElement ? 'part="bullet"' : ''} class="${e.bulletClass}"></${e.bulletElement}>`);
		}
		'fraction' === e.type &&
			(i = e.renderFraction
		...
		'string' == typeof e.el &&
			...
			s &&
				...
				(a.params.uniqueNavElements &&
					...
					s.length > 1 &&
						(s = s.filter(
							(e) => elementParents(e, '.swiper')[0] === a.el,
						)[0])),
		...
		l('click', (e, s) => {
			...
			if (
				...
				t(!0 === e ? 'paginationShow' : 'paginationHide'),
					i.forEach((e) =>
						e.classList.toggle(a.params.pagination.hiddenClass),
					);
			}
		});
	const b = () => {
