...
function Scrollbar(s) {
	...
	function b() {
		...
		const { scrollbar: s, rtlTranslate: e } = l,
			{ dragEl: a, el: r } = s,
	...
	function u() {
		...
		(e.style.width = ''),
			(e.style.height = ''),
			(i = l.isHorizontal() ? a.offsetWidth : a.offsetHeight),
			(c =
				l.size /
				(l.virtualSize +
			...
			l.params.scrollbar.hide && (a.style.opacity = 0),
			l.params.watchOverflow &&
				l.enabled &&
				s.el.classList[l.isLocked ? 'add' : 'remove'](
	...
	function g(s) {
		const { scrollbar: e, rtlTranslate: a } = l,
			{ el: r } = e;
		let t;
		(t =
			(f(s) -
				elementOffset(r)[l.isHorizontal() ? 'left' : 'top'] -
				(null !== o ? o : n / 2)) /
			(i - n)),
	...
	function h(s) {
		const { scrollbar: e, wrapperEl: a } = l,
			{ el: t, dragEl: o } = e;
		p &&
	...
	function T(s) {
		...
		p &&
			...
			e.snapOnRelease && l.slideToClosest());
	}
	function v(s) {
		const { scrollbar: e, params: a } = l,
			r = e.el;
		if (!r) return;
		const o = r,
	...
	function D() {
		...
		l.params.scrollbar = createElementIfNotDefined(
			l,
		...
		l.params.uniqueNavElements &&
			'string' == typeof a.el &&
			r.length > 1 &&
			1 === e.querySelectorAll(a.el).length &&
			(r = e.querySelector(a.el)),
			r.length > 0 && (r = r[0]),
			r.classList.add(
			...
			r &&
				((o = r.querySelector(
					classesToSelector(l.params.scrollbar.dragClass),
				)),
				o ||
					((o = createElement('div', l.params.scrollbar.dragClass)),
					r.append(o))),
			Object.assign(s, { el: r, dragEl: o }),
			a.draggable && l.params.scrollbar.el && l.scrollbar.el && v('on'),
			r &&
				r.classList[l.enabled ? 'remove' : 'add'](
					...classesToTokens(l.params.scrollbar.lockClass),
				);
	}
	function C() {
		...
		e &&
			...
			l.params.scrollbar.el && l.scrollbar.el && v('off');
	}
	e({
		scrollbar: {
			el: null,
		...
		a('changeDirection', () => {
			...
			(e = makeElementsArray(e)),
				e.forEach((e) => {
					e.classList.remove(s.horizontalClass, s.verticalClass),
						e.classList.add(
							l.isHorizontal()
								? s.horizontalClass
								: s.verticalClass,
		...
		a('setTransition', (s, e) => {
			!(function (s) {
				l.params.scrollbar.el &&
					l.scrollbar.el &&
					(l.scrollbar.dragEl.style.transitionDuration = `${s}ms`);
			})(e);
		}),
		a('enable disable', () => {
			const { el: s } = l.scrollbar;
			s &&
				s.classList[l.enabled ? 'remove' : 'add'](
					...classesToTokens(l.params.scrollbar.lockClass),
				);
		}),
		a('destroy', () => {
	...
	const E = () => {
		l.el.classList.add(
			...classesToTokens(l.params.scrollbar.scrollbarDisabledClass),
		),
			l.scrollbar.el &&
