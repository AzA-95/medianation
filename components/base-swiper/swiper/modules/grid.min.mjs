function Grid(e) {
	let i,
		...
		a,
		t,
		{ swiper: s, extendParams: l, on: o } = e;
	...
	o('init', () => {
		...
		o('update', () => {
			...
			t && !r
				...
				: !t &&
					r &&
					(i.classList.add(`${e.containerModifierClass}grid`),
					'column' === e.grid.fill &&
						i.classList.add(
							`${e.containerModifierClass}grid-column`,
						),
		...
		(s.grid = {
			initSlides: (e) => {
				const { slidesPerView: t } = s.params,
					{ rows: l, fill: o } = s.params.grid,
					n =
				...
				(a = Math.floor(n / l)),
					(i =
						Math.floor(n / l) === n / l ? n : Math.ceil(n / l) * l),
					'auto' !== t && 'row' === o && (i = Math.max(i, t * l)),
					(r = i / l);
			},
			unsetSlides: () => {
				s.slides &&
				...
				if ('row' === c && o > 1) {
					const r = Math.floor(e / (o * p)),
						...
						s =
							0 === r
								? o
								: Math.min(Math.ceil((g - r * p * o) / p), o);
					(m = Math.floor(a / s)),
						(h = a - m * s + r * o),
						(u = h + (m * i) / p),
						(t.style.order = u);
					...
					'column' === c
						? ((h = Math.floor(e / p)),
							...
							(h > a || (h === a && m === p - 1)) &&
								((m += 1), m >= p && ((m = 0), (h += 1))))
						: ((m = Math.floor(e / r)), (h = e - m * r));
				(t.row = m),
					(t.column = h),
					(t.style.height = `calc((100% - ${(p - 1) * d}px) / ${p})`),
					(t.style[s.getDirectionLabel('margin-top')] =
						0 !== m ? d && `${d}px` : ''),
					(t.swiperSlideGridSet = !0);
			},
			updateWrapperSize: (e, r) => {
				const { centeredSlides: a, roundLengths: t } = s.params,
					l = n(),
				...
				if (
					...
					for (let i = 0; i < r.length; i += 1) {
						let a = r[i];
						t && (a = Math.floor(a)),
							r[i] < s.virtualSize + r[0] && e.push(a);
					}
